...
sudo -E PATH=$GOPATH/bin:$PATH go run hack/e2e.go -- -v --test --test_args="--ginkgo.trace=true --ginkgo.seed=1378936983 --report-dir=/opt/stack/logs/ --ginkgo.v --ginkgo.skip=$(test_names)"
popd >/dev/null