...
function install_k8s_cloud_provider {
    ...
    cd $K8S_SRC
    sudo -E PATH=$PATH hack/local-up-cluster.sh 2>&1 | sudo tee $LOG_DIR/run-k8s.log
EOF
    sudo chmod +x ${K8S_SRC}/run-k8s.sh
    sudo cat ${K8S_SRC}/run-k8s.sh

    run_process kubernetes ${K8S_SRC}/run-k8s.sh
    popd >/dev/null
}
