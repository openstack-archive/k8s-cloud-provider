...
function install_k8s_cloud_provider {
    ...
    if [ ! -d "${K8S_SRC}" ]; then
        ...
        sudo git pr origin 45161;head=$(sudo git rev-parse HEAD);sudo git checkout master;sudo git cherry-pick $head;
        sudo git pr origin 45203;head=$(sudo git rev-parse HEAD);sudo git checkout master;sudo git cherry-pick $head;
        sudo git pr origin 45230;head=$(sudo git rev-parse HEAD);sudo git checkout master;sudo git cherry-pick $head;
